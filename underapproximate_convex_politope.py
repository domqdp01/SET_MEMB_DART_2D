...
def underapproximate_convex_polytope(vertices, directions):
    if len(vertices) == 0:
        raise ValueError("Error: 'vertices' list is empty!")
    if len(directions) == 0:
        raise ValueError("Error: 'directions' list is empty!")
    
    dim = directions[0].shape[0]
    for vertex in vertices:
        if vertex.shape[0] != dim:
            raise ValueError(f"Error: Vertex size {vertex.shape[0]} does not match expected size {dim}")
    ...
    for direction_index, direction in enumerate(directions):
        ...
        extremum_vertex = None
        
        for vertex in vertices:
            projection = np.dot(vertex, direction)
            if projection > max_projection:
                max_projection = projection
                extremum_vertex = vertex
        
        extremum_vertices.append(extremum_vertex)
        extremum_vertex_direction_indices.append(direction_index)
    
    Hp = np.zeros((len(directions), dim))
    hp = np.zeros(len(directions))
    
    for i, direction_index in enumerate(extremum_vertex_direction_indices):
        Hp[i, :] = directions[direction_index]
        hp[i] = np.dot(directions[direction_index], extremum_vertices[i])
